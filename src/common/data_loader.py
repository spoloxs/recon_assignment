...
def normalize_columns(df, source_name):
    ...
    """
    logger.debug(f"Normalizing {source_name} data...")
    
    if 'date' in df.columns:
        df['date'] = pd.to_datetime(df['date'])
    else:
        logger.error(f"{source_name} missing 'date' column")
...
def load_data():
    ...
    logger.info(f"Loading data from {os.path.dirname(BANK_FILE)}")
    ...
    try:
        ...
        logger.info(f"Loaded {len(bank_df)} bank transactions and {len(reg_df)} register transactions")
